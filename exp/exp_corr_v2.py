...
parser.add_argument("--dist", type=str, default="power")
parser.add_argument("--rho", type=float, default=0.0)
parser.add_argument("--block_size", type=int, default=10)
parser.add_argument("--seed", type=int, default=42)
parser.add_argument("--alpha_data", type=float, default=1.2,
                    help="power law exponent for feature frequencies")

args = parser.parse_args()

torch.manual_seed(args.seed)

def probability(name, n, alpha=1.2):
    if name == "exponential":
        prob = torch.exp(-torch.arange(n) / 1000)
    elif name == "power":
        prob = torch.tensor([1.0 / i ** alpha for i in range(1, n+1)])
    elif name == "linear":
...
prob = probability(args.dist, args.n, args.alpha_data).to(device)
criteria = nn.MSELoss()
...
def get_lr(step, n_steps, warmup_ratio=0.05):
    ...
    return (1.0 - min_lr) * 0.5 * (
        1 + math.cos(math.pi * (step - warmup_steps) /
        (n_steps - warmup_steps))) + min_lr
...
for m_i, m in enumerate(m_ran):
    m = m.item()
    torch.manual_seed(args.seed + m_i)
    model = torch.compile(FeatureRecovery(args.n, m)).to(device)
    parameter_groups = [
        {"params": model.W, "weight_decay": args.weight_decay,
         "lr": args.lr * (8 / m) ** 0.25},
    ...
    for step in range(args.n_steps):
        ...
        x = generate_correlated_batch(
            args.batch_size, args.n, prob,
            args.rho, device, args.block_size)
        for pg in optimizer.param_groups:
    ...
    print(f"rho={args.rho}, seed={args.seed}, alpha={args.alpha_data}, "
          f"m={m}, Loss={losses[m_i,-1]:.2e}, "
          f"Run time={time.perf_counter()-t0:.2f}s")

results["losses"] = losses
results["W"] = Ws
results["rho"] = args.rho
results["seed"] = args.seed
results["alpha_data"] = args.alpha_data
